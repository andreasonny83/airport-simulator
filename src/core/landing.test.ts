...
const runway: Runway = {
  ...
  airfield: layoutAirfield("red", { x: 50, y: 50 }, 0, 1, 0),
};
...
describe("checkLanding", () => {
  it("lands a matching plane crossing the threshold in the landing direction", () => {
    ...
    expect(checkLanding(plane, [runway])).toEqual({ type: "landed", runway });
  ...
  it("accepts headings across the ±PI seam", () => {
    ...
    expect(checkLanding(plane, [west])).toEqual({ type: "landed", runway: west });
  });
});
